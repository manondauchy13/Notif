...
import fr.logkey.notif.SendNotificationPack.*
...
class SendNotifActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        send.setOnClickListener(View.OnClickListener {
            FirebaseDatabase.getInstance().getReference("testnotif-3adf2").child("Tokens").child(UserTB.getText().toString().trim()).child("token").addListenerForSingleValueEvent(object : ValueEventListener {
    ...
    private fun UpdateToken(){
        ...
        var token: fr.logkey.notif.SendNotificationPack.Token = Token(refreshToken)
            ...
            override fun onResponse(call: Call<MyResponse?>, response: Response<MyResponse?>) {
                if (response.code() === 200) {
                    if (response.body()!!.success !== 1) {
                        Toast.makeText(this@SendNotifActivity, "Failed ", Toast.LENGTH_LONG).show()
                    }
                }
            }
